...
def create_single_market_image(state_data, output_path, post_date):
    # एकाच राज्यासाठी परफेक्ट साईझ (कोणतीही रिकामी जागा उरणार नाही)
    W, H = 1080, 1650
    ...
    # 5. तळाची हिंदी ॲड (योग्य जागेवर सेट केलेली)
    ad_y = 1130
    draw.rounded_rectangle([40, ad_y, 1040, ad_y + 460], radius=15, outline="#fbbf24", width=3)
    ...
    # Green Box & Page name
    draw.rectangle([230, ad_y + 300, 290, ad_y + 360], fill="#22c55e")
    draw.text((320, ad_y + 305), "Facebook Page : GREEN SOURCE", font=market_font, fill="#4ade80")
    draw.text((320, ad_y + 350), "Link in description / Bio me di gayi hai", font=district_font, fill="#94a3b8")

    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    img.save(output_path)
    print(f"Single state image saved successfully: {output_path}")
...
def main():
    ...
    # प्रत्येक राज्यासाठी त्याची स्वतःची अचूक आणि सुंदर इमेज तयार करणे
    for state_item in states_data:
        state_name = str(state_item.get('state', 'market')).lower().replace(' ', '_')
        output_filename = f"generated_images/onion_rates_{state_name}.png"
        create_single_market_image(state_item, output_filename, post_date)
